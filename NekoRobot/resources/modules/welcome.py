...
def send(update, message, keyboard, backup_message):
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            msg = update.effective_message.reply_text(
                markdown_parser(
                    backup_message + "\nCatatan: Pesan saat ini memiliki URL yang tidak valid dalam salah satu tombolnya. Harap diperbarui."
                ),
                parse_mode=ParseMode.MARKDOWN,
                reply_to_message_id=reply,
            )
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    "Anjay Owner Join.", reply_to_message_id=reply
                )
                welcome_log = (
...
def welcome(update: Update, context: CallbackContext):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        update.effective_message.reply_text(
            f"Percakapan ini memiliki pengaturan selamat datang yang diatur ke: `{pref}`.\n"
            f"*Pesan selamat datang (belum diisi {{}}) is:*",
            parse_mode=ParseMode.MARKDOWN,
        )
...
def goodbye(update: Update, context: CallbackContext):
    ...
    if not args or args[0] == "noformat":
        ...
        update.effective_message.reply_text(
            f"Pesan Goodbye di ataur ke: `{pref}`.\n"
